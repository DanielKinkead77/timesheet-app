    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-8">
                ...
                <form method="POST" action="{{ route('time-sheets.store') }}">
                    @csrf
                        ...
                        <select id="project_id"
                            ...
                            @foreach($projects as $project)
                                <option value="{{ $project->id }}"
                                    {{ (old('project_id', $selectedProjectId) == $project->id)
                                        ? 'selected' : '' }}>
                                    {{ $project->department }}
                                </option>
                            @endforeach
                        ...
                        <select id="hourly_rate_id"
                            ...
                            @foreach($hourlyRates as $rate)
                                <option value="{{ $rate->id }}"
                                    {{ old('hourly_rate_id') == $rate->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-8">
                        ...
                        <textarea id="work_description"
                                  ...
                                  class="w-full border-gray-300 rounded-lg shadow-sm
                                         focus:ring-indigo-500 focus:border-indigo-500"
                                  placeholder="What did you work on?">{{ old('work_description') }}</textarea>
                    </div>
